 ...
 */
import { runAPXCore } from './src/core-runner.js';
...
async function main() {
    ...
    const coreInput = {
        ...
        exportFormats: argv['export-formats']?.split(',').map(f => f.trim()),
    ...
    try {
        ...
        for (const [apiKey, snippets] of Object.entries(result.artifacts.codeSnippets)) {
            ...
            fs.writeFileSync(path.join(codeSnippetsDir, `${safeKey}.json`), JSON.stringify(snippets, null, 2));
        }
        // Save TypeScript types
        if (result.artifacts.typescriptTypes) {
            fs.writeFileSync(path.join(outputDir, 'types.d.ts'), result.artifacts.typescriptTypes);
        }
        // Save test suites
        if (result.artifacts.testSuites.length > 0) {
            ...
            for (const suite of result.artifacts.testSuites) {
                ...
                fs.writeFileSync(path.join(testSuitesDir, filename), suite.code || '');
            }
        }
        // Save SDK packages
        ...
        if (result.artifacts.documentation.length > 0) {
            ...
            for (const doc of result.artifacts.documentation) {
                ...
                fs.writeFileSync(path.join(docsDir, filename), doc.content || '');
            }
        }
        // Save examples
        if (result.artifacts.examples.length > 0) {
            fs.writeFileSync(path.join(outputDir, 'examples.json'), JSON.stringify(result.artifacts.examples, null, 2));
        }
        // Save summary
        ...
        if (result.data.length > 0) {
            fs.writeFileSync(path.join(outputDir, 'data.json'), JSON.stringify(result.data, null, 2));
        }
        console.log('');
